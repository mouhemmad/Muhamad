...
exports.handler = async (event) => {
  ...
  const db = supabase();
  const { data: user, error: userErr } = await db.from("users").select("id,username,restaurant_name,subscription_status,selected_theme,trial_end,subscription_end,social_tiktok,social_instagram,social_snapchat,social_location,social_whatsapp,social_facebook,video_url,hero_image").eq("username", username).single();
