...
var require_dist3 = __commonJS({
  "node_modules/@supabase/storage-js/dist/index.cjs"(exports2) {
    ...
    function setHeader(headers2, name, value) {
      const result = _objectSpread2({}, headers2);
    ...
    var _getRequestParams = (method, options, parameters, body) => {
      ...
      if (isPlainObject(body)) {
        ...
        let contentType;
        for (const [key, value] of Object.entries(headers2)) if (key.toLowerCase() === "content-type") contentType = value;
      ...
      */
      async uploadOrUpdate(method, path, fileBody, fileOptions) {
        ...
        return _this.handleOperation(async () => {
          ...
          let headers2 = _objectSpread2(_objectSpread2({}, _this.headers), method === "POST" && { "x-upsert": String(options.upsert) });
          ...
          const data = await (method == "PUT" ? put : post)(_this.fetch, `${_this.url}/object/${_path}`, body, _objectSpread2({ headers: headers2 }, (options === null || options === void 0 ? void 0 : options.duplex) ? { duplex: options.duplex } : {}));
          return {
            path: cleanPath,
            id: data.Id,
            fullPath: data.Key
      ...
      */
      async uploadToSignedUrl(path, token, fileBody, fileOptions) {
        ...
        return _this3.handleOperation(async () => {
          let body;
          const options = _objectSpread2(_objectSpread2({}, DEFAULT_FILE_OPTIONS), fileOptions);
          let headers2 = _objectSpread2(_objectSpread2({}, _this3.headers), { "x-upsert": String(options.upsert) });
          const metadata = options.metadata;
          if (typeof Blob !== "undefined" && fileBody instanceof Blob) {
            body = new FormData();
            body.append("cacheControl", options.cacheControl);
          ...
          return {
            path: cleanPath,
            fullPath: (await put(_this3.fetch, url.toString(), body, _objectSpread2({ headers: headers2 }, (options === null || options === void 0 ? void 0 : options.duplex) ? { duplex: options.duplex } : {}))).Key
          };
        });
      }
      /**
      ...
      */
      async createSignedUploadUrl(path, options) {
        ...
        return _this4.handleOperation(async () => {
          ...
          const headers2 = _objectSpread2({}, _this4.headers);
    ...
    var StorageBucketApi = class extends BaseApiClient {
      constructor(url, headers2 = {}, fetch$1, opts) {
        ...
        const finalHeaders = _objectSpread2(_objectSpread2({}, DEFAULT_HEADERS), headers2);
    ...
    var VectorIndexApi = class extends BaseApiClient {
      ...
      constructor(url, headers2 = {}, fetch$1) {
        const finalUrl = url.replace(/\/$/, "");
        const finalHeaders = _objectSpread2(_objectSpread2({}, DEFAULT_HEADERS), {}, { "Content-Type": "application/json" }, headers2);
...
var require_GoTrueAdminApi = __commonJS({
  "node_modules/@supabase/auth-js/dist/main/GoTrueAdminApi.js"(exports2) {
    ...
    var GoTrueAdminApi = class {
       ...
       * ```
       */
      constructor({ url = "", headers: headers2 = {}, fetch: fetch2, experimental }) {
        this.url = url;
        this.headers = headers2;
  ...
  "node_modules/@supabase/supabase-js/dist/index.cjs"(exports2) {
    ...
    var fetchWithAuth = (supabaseKey, supabaseUrl, getAccessToken, customFetch, tracePropagationOptions) => {
      ...
      return async (input, init) => {
        ...
        let headers2 = new HeadersConstructor(init === null || init === void 0 ? void 0 : init.headers);
        ...
        return fetch$1(input, _objectSpread2(_objectSpread2({}, init), {}, { headers: headers2 }));
      };
    };
    ...
    var SupabaseClient = class {
      ...
      */
      ...
      _initSupabaseAuthClient({ autoRefreshToken, persistSession, detectSessionInUrl, storage, userStorage, storageKey, flowType, lock, debug, throwOnError, experimental, lockAcquireTimeout, skipAutoInitialize }, headers2, fetch$1) {
        const authHeaders = {
        ...
        return new SupabaseAuthClient({
          ...
          headers: _objectSpread2(_objectSpread2({}, authHeaders), headers2),
          storageKey,
          autoRefreshToken,
          persistSession,
          detectSessionInUrl,
...
var BUCKET = "kozhen-uploads";
var headers = { "Access-Control-Allow-Origin": "*", "Access-Control-Allow-Headers": "Content-Type,Authorization", "Content-Type": "application/json" };
exports.handler = async (event) => {
  ...
  if (!file || !filename) return { statusCode: 400, headers, body: JSON.stringify({ error: "\u0645\u0644\u0641 \u0645\u0637\u0644\u0648\u0628" }) };
  const base64Data = file.replace(/^data:[^;]+;base64,/, "");
  const buffer = Buffer.from(base64Data, "base64");
  if (buffer.length > 5 * 1024 * 1024) return { statusCode: 400, headers, body: JSON.stringify({ error: "\u0627\u0644\u062D\u062F \u0627\u0644\u0623\u0642\u0635\u0649 5MB" }) };
  const ext = filename.split(".").pop().toLowerCase();
  const unique = Date.now() + "_" + Math.random().toString(36).substr(2, 8) + "." + ext;
  ...
  const { error } = await db.storage.from(BUCKET).upload(unique, buffer, { contentType: mimeMap[ext] || "application/octet-stream", upsert: false });
  if (error) return { statusCode: 500, headers, body: JSON.stringify({ error: error.message.includes("bucket") ? "\u0623\u0646\u0634\u0626 bucket \u0628\u0627\u0633\u0645 kozhen-uploads \u0641\u064A Supabase Storage" : error.message }) };
  const { data: { publicUrl } } = db.storage.from(BUCKET).getPublicUrl(unique);
